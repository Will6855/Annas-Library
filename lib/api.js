const { searchBooks, getBookDetails, getPopularBooks, getZlibBookDetails } = require('./scraper');
 ...
 * @route GET /api/zlib-detail/:lang/:id/:hash
 */
async function getZlibDetail(req, res) {
  try {
    const { lang, id, hash } = req.params;
    const data = await getZlibBookDetails(lang, id, hash);
