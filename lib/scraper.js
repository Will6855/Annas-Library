...
async function requestWithRetry(service, urlBuilder) {
  ...
  try {
    ...
    // If successful or client error (404), return response
    // If server error (5xx), throw to trigger retry
    if (response.status < 500) return response;
    throw new Error(`Server error ${response.status}`);
  } catch (error) {
    console.warn(`[${service.toUpperCase()}] Request failed (${error.message}). Switching domain...`);
    domain = await refreshDomain(service);
    url = urlBuilder(domain);
    console.log(`[${service.toUpperCase()}] Retrying with: ${domain}`);
    return fetch(url);
  }
}
...
async function searchBooks(query, lang, content, category, page = 1) {
  ...
  const urlBuilder = (domain) => {
    let searchUrl = `${domain}/search?q=${encodeURIComponent(fullQuery)}`;
...
async function resolveZlibIdToMd5(zlibId) {
  const urlBuilder = (domain) => `${domain}/search?q=${encodeURIComponent(`"zlib:${zlibId}"`)}`;
...
async function getBookDetails(md5) {
  ...
  const urlBuilder = (domain) => `${domain}/md5/${actualMd5}`;
